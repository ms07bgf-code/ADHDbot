...
- **重要 — 定期報告を有効にすること**: 判定は「直近の点が半径150m以内に30分以上とどまっている」ことで行うため、静止中に位置を送らない設定（大幅移動検知のみ）だと滞在が成立しない。静止中も5〜10分間隔で送る設定にする。
  - 報告間隔が `CONFIG.DWELL.MAX_POINT_GAP_MINUTES`（既定15分）を超えると、その区間は滞在の連続性が認められず判定がリセットされる
...
| 報告欠落ガード | 最低3点 / 点の間隔15分以内 | `Config.js` `DWELL.MIN_POINTS`, `MAX_POINT_GAP_MINUTES` |
