...
| 位置ロガーアプリ（層B） | `{"secret": "...", "lat": 35.68, "lng": 139.76}` | `handleLocationPost` |
...
### 7. 位置ロガーアプリの設定（動的アンカー＝層B / §7）

長時間滞在した場所（カフェ・店・職場）から離れたときにリマインドを出すための設定。OwnTracks / Overland など、任意のURLへ位置をPOSTできるロガーアプリを使う。

- 送信先: 上記のWebアプリURL
- 本文: `{"secret": "<WEBHOOK_SECRETの値>", "lat": <緯度>, "lng": <経度>}`
- **報告間隔: 5分（決定値）**。静止中も5分おきに送る設定にする。判定は「直近の点が半径150m以内に30分以上とどまっている」ことで行うため、静止中に位置を送らない設定（大幅移動検知のみ）だと滞在が成立しない。
  - 5分間隔なら30分の滞在に約7点が入り、`MIN_POINTS`(3点) に対して十分な余裕がある
  - `MAX_POINT_GAP_MINUTES`(15分) は、5分間隔に対して**2回続けて欠落するまで許容**し、3回目で滞在の連続性を認めない、という意味になる
  - 離脱の検知は点が届いた瞬間にしか起きないため、報告間隔がそのままリマインドの遅れになる。5分間隔なら離脱判定半径(300m)を越えてから最大5分・徒歩で約400m進んだ時点で鳴る（この距離が文面切り替えの500mを下回るため、多くの場合は「単語のみ」＝取りに戻れる側の文面になる）
- 端末側には判断ロジックを一切持たせない。滞在判定・離脱判定はすべてGAS側で行う（§7 責務分担）
...
| ロガーの報告間隔 | 5分 | ロガーアプリ側の設定（コードではなく端末側） |
